...
from PySide6.QtGui import QColor, QIcon, QImage, QPainter, QPixmap, QTextOption
    ...
    def capture_viewport_thumbnail(self, target_width=1200, use_high_quality=True):
        ...
        try:
            ...
            if use_high_quality:
                # 尝试使用高质量渲染方法
                try:
                    # 使用viewport的GetDIB方法，可以获得更高分辨率的图片
                    viewport = mxs.viewport.getViewportDib()
                    if viewport:
                        # 获取视口尺寸
                        view_size = mxs.viewport.GetViewSize()
                        vp_width = view_size[1]
                        vp_height = view_size[2]
                        
                        # 使用更高的分辨率（基于当前视口大小，但放大）
                        render_width = max(target_width, vp_width * 2)
                        render_height = int(render_width * (vp_height / float(vp_width)))
            ...
            if os.path.exists(temp_full_path):
                ...
                if not full_image.isNull():
                    ...
                    if not cropped_image.isNull():
                        # 缩放到目标尺寸（保持宽高比）
                        aspect_ratio = crop_width / float(crop_height)
                        if aspect_ratio > 1:
                            # 横向
                            target_height = int(target_width / aspect_ratio)
                        else:
                            ...
                            target_height = target_width // aspect_ratio if aspect_ratio > 0 else target_width
